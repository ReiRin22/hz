...
digraph process {
    ...
    "実装全体の final code reviewer subagent を派遣" -> "worktree を使用した?" [label="" shape=diamond];
    "worktree を使用した?" -> "superpowers:finishing-a-development-branch を使う" [label="yes"];
    "worktree を使用した?" -> "手動コミット・プッシュを案内" [label="no" shape=box];
}
```
...
- **using-git-worktrees** - 分離ワークスペースを保証する
- **writing-plans** - このスキルが実行する計画を作る
- **plan-progress-viewer** - Phase プランファイル実行時に進捗を可視化
- **requesting-code-review** - reviewer subagents 用コードレビューテンプレート
...
- **executing-plans** - 同一セッションではなく並列セッションで実行する時に使う
