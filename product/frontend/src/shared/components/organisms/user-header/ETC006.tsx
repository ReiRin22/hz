...
// カスタムフック
import { usePatientData } from "@/shared/hooks/user-header/usePatientData";
import { useAppEventHandlers } from "@/shared/hooks/user-header/useAppEventHandlers";
import { useUserAlerts } from "@/shared/hooks/user-header/useUserAlerts";
import { useAutoLogout } from "@/shared/hooks/user-header/useAutoLogout";

// レイアウトコンポーネント（直接インポート）
import { GlobalHeader } from "@/shared/components/organisms/user-header/GlobalHeader";
import { PatientHeader } from "@/shared/components/organisms/user-header/PatientHeader";

// 医療機能コンポーネント
import { ExternalMedicalRecordsDialog } from "@/shared/components/organisms/user-header/ExternalMedicalRecordsDialog";
import { HealthCheckupDialog } from "@/shared/components/organisms/user-header/HealthCheckupDialog";
import { SetManagementDashboard } from "@/shared/components/organisms/user-header/SetManagementDashboard";

// その他のコンポーネント
import { NewPatientBadge } from "@/shared/components/organisms/user-header/NewPatientBadge";
...
// 定数
import { currentUser, patientDatabase } from "@/shared/assets/user-header/medical-data";

// 型定義
import type { Order } from "@/shared/types/user-header";
import type { RegisteredSet, SetApplyOptions } from "@/shared/types/user-header/set-registration-types";

export default function ETC006Page() {
  // カスタムフックの使用
  const {
    currentPatient,
    progressRecords,
  ...
  useEffect(() => {
    // 既存のテーマカラークラスを削除
    const themeClasses = ['theme-blue', 'theme-green', 'theme-purple', 'theme-pink', 'theme-orange', 'theme-red', 'theme-white', 'theme-black'];
