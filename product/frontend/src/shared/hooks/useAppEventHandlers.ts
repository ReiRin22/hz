...
import { formatDateToJapanese, isNewPatient } from "../utils/patient-utils";
import {
...
import type { Patient, CurrentRecord, MedicalRecord } from "../types/patient-types";
// TODO: src/_shared/constants/medical-data.ts が実装されたら以下の import を有効化する
// import { patientDatabase } from "../constants/medical-data";
...
export function useAppEventHandlers({
  ...
  const handlePatientSelect = (patientId: string) => {
    ...
    if (success) {
      ...
      // TODO: patientDatabase が実装されたら患者情報を toast に表示する
      // const patient = patientDatabase[patientId];
      toast.success(`${patientType}に切り替えました`);
      return true;
    } else {
  ...
  // 病名登録の処理
  // TODO: 診断登録フォームの型定義が確定したら DiagnosisFormData 等の型に置き換える
