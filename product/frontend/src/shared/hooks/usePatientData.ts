...
// import { defaultPatient, sampleProgressRecords, sampleHandoverItems } from "../constants/medical-data";
// TODO: src/_shared/constants/medical-data.ts が実装されたら defaultPatient を削除して patientDatabase から取得する
const defaultPatient: Patient = {
  ...
  insurance: { type: "", number: "", burden: "" },
};
const sampleProgressRecords: ProgressRecord[] = [];
const sampleHandoverItems: HandoverItem[] = [];
...
export const usePatientData = () => {
  ...
  // 経過記録の追加
  const addProgressRecord = (record: Omit<ProgressRecord, 'id' | 'date' | 'time'>) => {
    const now = new Date();
  ...
  // 申し送りの追加
  const addHandoverItem = (item: Omit<HandoverItem, 'id' | 'date' | 'time'>) => {
    const now = new Date();
