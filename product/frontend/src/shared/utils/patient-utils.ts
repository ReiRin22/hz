import type { Patient, MedicalAlert, MedicalRecord, MedicationRecord, ExternalMedicalRecord, HealthCheckupRecord, TestResult, StatsData } from "../types/patient-types";
// TODO: src/_shared/constants/medical-data.ts が実装されたら以下の import を有効化し、stub を削除する
// import {
//   patientDatabase,
//   newPatientAlerts,
...
// TODO: src/_shared/constants/medical-data.ts が実装されたら以下 stub をすべて削除する
const patientDatabase: Record<string, Patient> = {};
const newPatientAlerts: MedicalAlert[] = [];
const existingPatientAlerts: MedicalAlert[] = [];
const newPatientTestResults: TestResult[] = [];
const sampleTestResults: TestResult[] = [];
const imageDatabase: Record<string, number> = {};
const testResultsDatabase: Record<string, TestResult[]> = {};
const recordsDatabase: Record<string, MedicalRecord[]> = {};
const medicationDatabase: Record<string, MedicationRecord[]> = {};
const externalMedicalRecordsDatabase: Record<string, ExternalMedicalRecord[]> = {};
const healthCheckupDatabase: Record<string, HealthCheckupRecord[]> = {};
...
// 現在の患者の診療記録を取得
export const getCurrentPatientRecords = (patientId: string): MedicalRecord[] => {
  return recordsDatabase[patientId] || [];
};

// 現在の患者の薬歴を取得
export const getCurrentPatientMedications = (patientId: string): MedicationRecord[] => {
  return medicationDatabase[patientId] || [];
};

// 患者の他院診療情報を取得
export const getCurrentPatientExternalRecords = (patientId: string): ExternalMedicalRecord[] => {
  return externalMedicalRecordsDatabase[patientId] || [];
};

// 患者の健診情報を取得
export const getCurrentPatientHealthCheckups = (patientId: string): HealthCheckupRecord[] => {
...
export const getGenderColor = (gender: string): string => {
  return gender === "男性" ? "bg-medical-primary" : "bg-pink-500";
};