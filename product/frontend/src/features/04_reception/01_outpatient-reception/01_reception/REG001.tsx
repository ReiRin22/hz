...
import { Sheet, SheetContent, SheetDescription, SheetHeader, SheetTitle, SheetTrigger } from '@shared/components/atoms/sheet';
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from '@shared/components/atoms/select';
import { CalendarIcon, Users, Calendar as CalendarTabIcon, Filter, Clock, Search } from 'lucide-react';
import { Header } from './components/molecules/Header';
import { PatientReception } from './components/molecules/PatientReception';
import { UnifiedAppointments } from './components/molecules/UnifiedAppointments';
import { QuestionnaireDialog } from './components/molecules/QuestionnaireDialog';
import { QuestionnaireInputDialog } from './components/molecules/QuestionnaireInputDialog';
import { ReceptionDialog } from './components/molecules/ReceptionDialog';
import { WalkInReceptionDialog } from './components/molecules/WalkInReceptionDialog';
import { PatientSearchDialog } from './components/molecules/PatientSearchDialog';
import { Tabs, TabsContent, TabsList, TabsTrigger } from '@shared/components/atoms/tabs';

export default function REG001Page() {
  ...
  const handleReceptionDialogReception = (data: any) => {
    ...
    // 受付番号を生成（その日の最大番号+1）
    const todayReceptions = receptionedPatients.filter(p => p.receptionDate === selectedDateString);
    const maxReceptionNo = todayReceptions.reduce((max, p) => {
      const num = parseInt(p.receptionNo || '0');
      return num > max ? num : max;
    }, 0);
    const newReceptionNo = String(maxReceptionNo + 1).padStart(3, '0');
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="p-6">
        <div className="max-w-7xl mx-auto space-y-6">
          ...
          {/* 受付状況（主画面）と予約状況（サイドパネル） */}
          <div className="flex gap-4 mb-4 relative">
            <div className="flex-1">
              <Button 
                variant="outline" 
                onClick={() => setIsAppointmentsPanelOpen(!isAppointmentsPanelOpen)}
              >
                <CalendarTabIcon className="mr-2 h-4 w-4" />
                予約状況を表示 ({totalAppointments})
              </Button>
            </div>
          </div>
          ...
          {/* 予約状況サイドバー */}
          <div 
            className={`fixed right-0 top-[320px] w-[60vw] h-[50vh] bg-background border-l border-t shadow-2xl z-50 overflow-hidden flex flex-col transition-transform duration-300 ease-in-out ${
              isAppointmentsPanelOpen ? 'translate-x-0' : 'translate-x-full'
            }`}
          >
            <div className="flex items-center justify-between p-4 border-b bg-background">
              <h3 className="flex items-center gap-2 font-semibold text-lg">
                <CalendarTabIcon className="h-5 w-5" />
