  ...
  return (
    <Card style={{
      ...
      <CardHeader style={{
        ...
        }}>{t.bedManagement.chartTitle}</CardTitle>
      </CardHeader>
      <CardContent style={{
        backgroundColor: theme?.value === 'black' ? '#0D0D0D' : undefined
      }}>
        <ResponsiveContainer width="100%" height={200}>
          <BarChart data={data}>
            ...
            />
            <YAxis 
              label={{ value: t.bedManagement.chartYLabel, angle: -90, position: 'insideLeft', style: { fontSize: 11, fill: theme?.value === 'black' ? '#F9FAFB' : undefined } }}
              tick={{ fontSize: 11, fill: theme?.value === 'black' ? '#F9FAFB' : undefined }}
              stroke={theme?.value === 'black' ? '#6B7280' : undefined}
            />
            <Tooltip 
