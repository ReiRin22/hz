...
interface OrderDetail {
  ...
  contrastAgent?: string; // 造影剤の種類（検体検査で使用）
  groupId?: string; // グループID（セットや履歴から追加された場合）
  groupName?: string; // グループ名（例：「肝機能パネル」「2024-12-15 風邪症状」）
  groupType?: 'set' | 'history'; // グループの種類
  itemCode?: string; // 元の項目コード（検査項目の場合はlab_ast等）
  subcategory?: string; // サブカテゴリID（検査項目の場合）
  subcategoryName?: string; // サブカテゴリ名（検査項目の場合）
}
