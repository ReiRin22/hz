...
interface RightSideMenuState {
  ...
  menuFetchError: string | null;

  toggleCollapse: () => void;
  setItems: (items: RightSideMenuItemResponse[]) => void;
  setMenuFetchError: (error: string | null) => void;
...
const initialState = {
  ...
  selectedDepartments: [],
  confirmedMemos: {},
  items: [] as RightSideMenuItemResponse[],
  menuFetchError: null as string | null,
};
...
export const useRightSideMenuStore = create<RightSideMenuState>((set) => ({
  ...
  setMenuFetchError: (error) => set({ menuFetchError: error }),
