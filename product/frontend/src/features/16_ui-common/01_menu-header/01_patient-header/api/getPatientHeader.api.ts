import type { GetPatientHeaderResponse, PatientHeaderResponse } from '@/front_bff_shared/features/karte/patientHeader/types/response/patient-header.api.response';
...
export async function getPatientHeader(
  ...
  const res = await fetch(
    `${BFF_BASE_URL}/api/patients/${encodeURIComponent(patientId)}/header`,
    {
      cache: 'no-store',
      signal,
    }
