import { Shield, ShieldCheck, ShieldX, Info, MessageCircle } from "lucide-react";
import { Badge } from "@/shared/components/atoms/badge";
import { Tooltip, TooltipContent, TooltipProvider, TooltipTrigger } from "@/shared/components/atoms/tooltip";

export type MedicalInfoSharingStatus = "full-consent" | "partial-consent" | "verbal-consent" | "no-consent";
...
export function MedicalInfoSharingBadge({
  ...
  // プライバシーモード時は常に「一部同意」として表示
  const displayStatus = isPrivacyMode ? "partial-consent" : data.status;
      ...
      <Tooltip>
        <TooltipTrigger asChild>
          <div className="inline-block">
            <Badge
              variant="outline"
              className={`inline-flex items-center space-x-1.5 px-2.5 py-1 border cursor-pointer hover:shadow-md hover:scale-[1.02] transition-all duration-200 ${statusInfo.color} ${className}`}
