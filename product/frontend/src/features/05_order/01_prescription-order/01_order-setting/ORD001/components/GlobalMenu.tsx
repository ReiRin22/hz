...
const getMenuItems = (currentView: string) => [
  ...
  { id: 'consultation', label: '他科依頼', icon: Users, active: currentView === 'consultation' },
  { id: 'patient', label: '患者情報', icon: User, active: currentView === 'patient' },
  { id: 'document', label: '文書', icon: FileText, active: currentView === 'document' },
...
interface GlobalMenuProps {
  ...
  currentView?: 'order' | 'patient' | 'appointment' | 'chart' | 'external-info' | 'consultation' | 'document' | 'results';
...
export function GlobalMenu({ 
  ...
  return (
    <div className="w-25 bg-sidebar border-r border-sidebar-border flex flex-col h-full">
      {/* 患者情報表示 */}
            ...
            <div key={item.id}>
              <div
                ...
                onClick={() => {
                  ...
                  } else if (item.id === 'consultation') {
                    onMenuClick?.('consultation');
                  } else if (item.id === 'document') {
