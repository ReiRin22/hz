...
import type { Order, AllergyCategory } from '../../types';
...
export function AllergyDetailDialog({ open, onClose, order }: AllergyDetailDialogProps) {
  ...
  // アレルギーをカテゴリ別にグループ化
  const groupedAllergies = order.allergies.reduce((acc, allergy) => {
    ...
    return acc;
  }, {} as Record<AllergyCategory, typeof order.allergies>);

  // カテゴリの表示順
  const categoryOrder: AllergyCategory[] = ['薬剤', '食事', '環境', 'ラテックス', '金属', 'その他'];
                ...
                <Table>
                  ...
                  <TableBody>
                    {categoryOrder.map((category) => {
                      const allergiesInCategory = groupedAllergies[category];
                      if (!allergiesInCategory || allergiesInCategory.length === 0) return null;
