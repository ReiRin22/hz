...
import { ContraindicationDialog } from './components/molecules/ContraindicationDialog';
...
export default function DEP008Page() {
  ...
  const [contraindicationDialogOpen, setContraindicationDialogOpen] = useState(false);
  ...
  // ステータスの順序定義
  const statusOrder = ['指示受済', '実施済', '結果待ち', '実施済み', '出庫'];
  ...
  const handleSearch = (criteria: SearchFilters) => {
    ...
    // ステータスフィルタ
    if (criteria.statusFrom || criteria.statusTo) {
      const fromIndex = criteria.statusFrom ? statusOrder.indexOf(criteria.statusFrom) : 0;
      const toIndex = criteria.statusTo ? statusOrder.indexOf(criteria.statusTo) : statusOrder.length - 1;
      
      filtered = filtered.filter(order => {
        const orderStatusIndex = statusOrder.indexOf(order.status);
        return orderStatusIndex >= fromIndex && orderStatusIndex <= toIndex;
      });
    }
  ...
  const handleAccept = (orderId: string) => {
      ...
      prevOrders.map(order =>
        order.id === orderId
          ? {
              ...
              statusHistory: [
                ...(order.statusHistory || []),
                { status: '実施済' as const, timestamp, updatedBy: currentUser }
              ]
            }
          : order
      )
    ...
    toast.success('オーダーを実施しました');
  };

  // 検査依頼処理（病理・細菌：実施→結果待ち）
  ...
  const handleImplementerSave = (data: ImplementerInput) => {
    ...
    // ステータス判定：注射オーダー・薬剤・処方は「出庫」、それ以外は「実施済み」
    const newStatus: Order['status'] = 
      ['注射オーダー', '薬剤', '処方'].includes(selectedOrder.orderType) ? '出庫' : '実施済み';
  ...
  // 禁忌/特記事項詳細表示
  const handleContraindicationClick = (order: Order) => {
    setSelectedOrder(order);
    setContraindicationDialogOpen(true);
  };
  ...
  const handleResultSave = (results: TestResult[], notes: string) => {
    ...
    if (criticalCount > 0) {
      toast.error(`検査結果を存しました。緊急異常値が${criticalCount}件あります！`, {
        duration: 5000
      });
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <header className="bg-white border-b border-gray-200 shadow-sm">
        <div className="max-w-[1600px] mx-auto px-6 py-4">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-gray-900">透析指示受け一覧</h1>
      ...
      <main className="max-w-[1600px] mx-auto px-6 py-6 space-y-6">
        ...
        <div>
          ...
          <OrderTable
            ...
            onContraindicationClick={handleContraindicationClick}
      ...
      <ContraindicationDialog
        open={contraindicationDialogOpen}
        onClose={() => setContraindicationDialogOpen(false)}
        order={selectedOrder}
