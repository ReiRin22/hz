...
// 患者別の入院エピソードデータベース
export const hospitalizationDatabase: Record<string, HospitalizationEpisode[]> = {
  "P123456789": [
    {
      id: "hosp_001",
...
export const sampleProgressRecords: ProgressRecord[] = [
  {
    ...
    department: "循環器内科",
    isImportant: true,
    schema: "https://images.unsplash.com/photo-1532153470116-e8c2088b8ac1?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3Nzg4Nzd8MHwxfHNlYXJjaHwxfHxtZWRpY2FsJTIwZGlhZ3JhbSUyMGFuYXRvbXl8ZW58MXx8fHwxNzY0NTk4NTM2fDA&ixlib=rb-4.1.0&q=80&w=400",
  },
  {
    id: "prog2",
    date: "2024/12/27",
  ...
  },
  {
    ...
    department: "循環器内科",
    isImportant: false,
    schema: "https://images.unsplash.com/photo-1725398753305-810d9e245a80?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3Nzg4Nzd8MHwxfHNlYXJjaHwxfHxib2R5JTIwY2hhcnQlMjBza2V0Y2h8ZW58MXx8fHwxNzY0NjM5MzUzfDA&ixlib=rb-4.1.0&q=80&w=400",
  },
  {
    id: "prog4",
    date: "2024/12/26",
    ...
    department: "循環器内科",
    isImportant: true,
    schema: "https://images.unsplash.com/photo-1725398467967-c27544f7a4b5?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3Nzg4Nzd8MHwxfHNlYXJjaHwxfHxhbmF0b21pY2FsJTIwaWxsdXN0cmF0aW9ufGVufDF8fHx8MTc2NDYzOTM1Mnww&ixlib=rb-4.1.0&q=80&w=400",
  },
];
...
export const testResultsDatabase: Record<string, TestResult[]> = {
  ...
  "P001234567": [], // 新患は検査結果なし
  "P002345678": [], // 新患は検査結果なし
};
...
次回薬剤師面談：1週間後`,
    ...
    // 2024/12/27の記録
    {
      ...
      visitType: "inpatient",
      title: "胸痛症状の改善確認",
      content: "前日からの胸痛症状軽減を確認。血圧も安定してきている。",
      author: "田中 医師（循環器内科）",
      insurance: { type: "社保", burden: "3割" },
      soapRecord: `S (Subjective - 主観的情報):
昨日からの胸痛症状は軽減している
...
アムロジピン5mg開始`,
    ...
    // 2024/12/26の処方オーダー（初診と同日）
    {
      ...
      type: "prescription",
      visitType: "outpatient",
      title: "初診時処方（降圧薬・抗狭心症薬）",
      content: "アムロジピン5mg 1日1回朝食後、ニトロール5mg 舌下錠頓用を処方。",
      author: "田中 医師（循環器内科）",
      insurance: { type: "社保", burden: "3割" },
      soapRecord: `1. アムロジピン錠5mg 1錠
   用法：1日1回朝食後
   日数：28日分
...
・副作用（めまい、頭痛等）に注意`,
    ...
    // 2024/12/26の検査オーダー（初診と同日）
    {
      id: "rec_p123_1c",
      date: "2024/12/26",
      time: "09:50",
      type: "test",
      title: "心臓精査（心エコー・血液検査）オーダー",
      content: "心エコー検査、心筋マーカー含む血液検査をオーダー。",
      author: "田中 医師（循環器内科）",
      insurance: { type: "社保", burden: "3割" },
      soapRecord: `1. 心エコー検査
   目的：心機能評価、壁運動評価
...
2. 血液検査
   項目：CK-MB、トロポニンI、BNP、脂質プロファイル
   目的：心筋障害マーカー確認
   
3. 胸部X線
   目的：心拡大、肺うっ血の有無確認`,
    },
    // 2024/12/02 - 同日に医師と看護師が記録した例
    {
      id: "rec_p123_dec2_1",
      date: "2024/12/02",
      time: "09:30",
      type: "vital",
      title: "バイタルサイン測定",
      content: "診察前のバイタルサイン測定を実施。異常値なし。",
      author: "山本 看護師（循環器内科）",
      insurance: { type: "社保", burden: "3割" },
      soapRecord: `バイタルサイン：
- 血圧：128/82 mmHg
...
全身状態：良好`,
    ...
    },
    {
      ...
      type: "progress",
      title: "風邪症状の診察",
      content: "3日前からの咳、鼻汁、咽頭痛を主訴に来院。発熱なし。",
      author: "鈴木 医師（循環器内科）",
      insurance: { type: "社保", burden: "3割" },
      soapRecord: `S (Subjective - 主観的情報):
3日前から咳と鼻汁が出現
...
O (Objective - 客観的情報):
バイタルサイン：BP 135/82mmHg, HR 74bpm, BT 36.8℃, SpO2 98%
咽頭：発赤軽度、扁桃腫大なし
肺音：清、wheezeなし
心音：整
...
症状悪化時は再診指示`,
    ...
    },
    {
      ...
      visitType: "outpatient",
      title: "感冒薬処方",
      content: "総合感冒薬、鎮咳去痰薬を処方。",
      author: "鈴木 医師（循環器内科）",
      insurance: { type: "社保", burden: "3割" },
      soapRecord: `1. PL配合顆粒 1包
   用法：1日3回毎食後
   日数：5日分
...
2. ムコダイン錠500mg 1錠
   ...
   日数：5日分`,
    },
    {
      ...
      type: "test",
      title: "インフルエンザ迅速検査",
      content: "インフルエンザ抗原検査実施。陰性確認。",
      author: "高橋 看護師（循環器内科）",
      insurance: { type: "社保", burden: "3割" },
      soapRecord: `検査項目：インフルエンザ抗原検査
検査結果：
・A型：陰性
・B型：陰性
...
- 次回受診日の確認`,
    ...
    // 2024/11/15 - 経過記録と処方変更の例
    {
      ...
      type: "progress",
      title: "高血圧フォローアップ",
      content: "血圧コントロール不良のため降圧薬調整。",
      author: "田中 医師（循環器内科）",
      insurance: { type: "社保", burden: "3割" },
      soapRecord: `S (Subjective - 主観的情報):
最近家庭血圧が高め（150/90mmHg前後）
頭重感を時々自覚
めまいやふらつきなし

O (Objective - 客観的情報):
バイタルサイン：BP 158/94mmHg, HR 78bpm, BT 36.6℃
心音：整、雑音なし
肺音：清
浮腫なし
...
2週間後再診`,
    ...
    },
    {
      ...
      visitType: "outpatient",
      title: "降圧薬処方変更",
      content: "アムロジピン5mg→10mgに増量。",
      author: "田中 医師（循環器内科）",
      insurance: { type: "社保", burden: "3割" },
      soapRecord: `1. アムロジピン錠10mg 1錠（5mgから増量）
   用法：1日1回朝食後
   日数：28日分
...
次回診察：1ヶ月後`,
    ...
    },
    {
      ...
      visitType: "inpatient",
      title: "ラシックス注静注",
      content: "心不全予防のためラシックス注20mg静注実施。",
      author: "高橋 看護師（循環器内科）",
      insurance: { type: "社保", burden: "3割" },
    },
    {
    ...
    // 2024/01/15 - 同日に医師・看護師・リハビリが記録した例（入院患者）
    {
      id: "rec_p123_jan15_1",
      date: "2024/01/15",
      time: "08:00",
      ...
      visitType: "inpatient",
      title: "朝のバイタル測定",
      content: "入院患者の朝のバイタルサイン測定。血圧やや高め。",
      author: "佐藤 看護師（循環器内科）",
      insurance: { type: "社保", burden: "3割" },
      soapRecord: `バイタルサイン（8:00測定）：
- 血圧：148/90 mmHg（やや高め）
- 脈拍：82 bpm
- 体温：36.5℃
- 呼吸数：18 回/分
...
- 特記事項：なし`,
    ...
    {
      id: "rec_p123_medical_doc1",
      date: "2024/12/18",
      time: "14:00",
      type: "medicalDocument",
      visitType: "inpatient",
      title: "診療情報提供書作成",
      content: "循環器専門病院宛の診療情報提供書を作成。冠動脈造影検査依頼。",
      author: "田中 医師（循環器内科）",
      insurance: { type: "社保", burden: "3割" },
    },
    {
    ...
    },
    {
      ...
      visitType: "outpatient",
      title: "他院紹介状（取込）",
      content: "A病院からの紹介状をスキャン取込。労作性狭心症疑いで精査依頼。",
      author: "田中 医師（循環器内科）",
      insurance: { type: "社保", burden: "3割" },
    },
    {
