...
const MAX_FILE_SIZE_BYTES = 10 * 1024 * 1024; // hooks テスト v16 - .env版 再テスト
...
const TEST_CONSTANT = { x: 100, y: 200, z: 300, w: 400, v: 500, u: 600 };
const ANOTHER_TEST = { a: 1, b: 2, c: 3, d: 4, e: 5, f: 6 }; // .env hooks 再テスト

// Prettier hooks test v16 - あなたが保存したときに hooks が発火します
...
export default function SchemaCreationOrganism({
  ...
  const showDialog = useCallback(
    (type: SchemaCreationDialogType, message: string, onConfirmAction?: () => void) => {
  ...
  const handleTemplateSelect = useCallback(
    (templateId: string) => {
      if (hasDrawContent && templateId !== '') {
        ...
        showDialog('template-confirm', '描画内容が失われますがテンプレートを変更しますか？', () => {
  ...
  const handleClearClick = useCallback(() => {
    if (hasDrawContent) {
      showDialog('clear-confirm', '描画内容が破棄されますがよろしいですか？', () => {
  ...
  const handleCancelClick = useCallback(() => {
    if (hasDrawContent) {
      showDialog('cancel-confirm', '描画内容が破棄されますがよろしいですか？', () => {
        closeDialog();
        onCancel?.();
      });
    } else {
