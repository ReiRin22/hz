...
interface DraggableCommentPopupProps {
  isOpen: boolean;
  onClose: () => void;
  myComments: Comment[];
  patientComments: Comment[];
  departmentComments: Comment[];
  onCommentSelect: (comment: Comment) => void;
  onMyCommentManagementOpen: () => void;
  soapContainerRef?: React.RefObject<HTMLDivElement>; // SOAPコンテナのref
}
  ...
  useEffect(() => {
    if (isOpen && popupRef.current) {
      ...
      if (soapContainerRef?.current) {
        ...
        // SOAPコンテナの右側に配置
        setPosition({
          x: soapRect.right + 12, // SOAP右端から12pxの余白
          y: soapRect.top, // SOAPと同じ高さ
        });
      } else {
        // フォールバック: 画面右側に表示
        setPosition({
          x: window.innerWidth - popupRect.width - 20,
          y: 100,
        });
      }
    }
        ...
        <div onMouseDown={(e) => e.stopPropagation()}>
          <Tabs defaultValue="my" className="w-full">
            ...
            <ScrollArea className="h-[300px]">
              {/* Myコメント */}
              <TabsContent value="my" className="m-0 p-1.5">
                  ...
                  {myComments.map((comment) => (
                    ...
                    >
                      <div className="flex items-start space-x-1.5">
                        <MessageSquare className="w-4 h-4 mt-0 text-blue-500 flex-shrink-0" />
              ...
              {/* 患者別コメント */}
              <TabsContent value="patient" className="m-0 p-1.5">
                <div className="space-y-0.5">
                  {patientComments.map((comment) => (
                    ...
                    >
                      <div className="flex items-start space-x-1.5">
                        <MessageSquare className="w-4 h-4 mt-0 text-green-500 flex-shrink-0" />
                        <div className="flex-1 min-w-0">
                          <div className="font-medium text-sm line-clamp-2 whitespace-pre-wrap">{comment.content}</div>
                        </div>
                      </div>
              ...
              {/* 診療科コメント */}
              <TabsContent value="department" className="m-0 p-1.5">
                <div className="space-y-0.5">
                  {departmentComments.map((comment) => (
                    ...
                    >
                      <div className="flex items-start space-x-1.5">
                        <MessageSquare className="w-4 h-4 mt-0 text-purple-500 flex-shrink-0" />
                        <div className="flex-1 min-w-0">
                          <div className="font-medium text-sm line-clamp-2 whitespace-pre-wrap">{comment.content}</div>
                        </div>
                      </div>
