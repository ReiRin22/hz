...
import { formatDateToJapanese, isNewPatient } from "../src/utils/patient-utils";
import { 
...
import { patientDatabase } from "../src/constants/medical-data";
...
export function useAppEventHandlers({
  ...
  const handleRecordSelect = (record: any) => {
    // 選択された記録のIDを設定（記録詳細パネルに表示するため）
    setSelectedRecord(record.id);
    
    toast.info(`${record.date} ${record.title}の記録を選択しました`, {
      description: '記録詳細を表示しています'
    });
  };

  // 記録編集（編集可能期間内の記録を入力欄に反映）
  const handleEditRecord = (record: any) => {
    ...
    toast.success(`${record.date}の記録を編集モードで開きました`, {
      description: '編集後に「確定」ボタンで保存してください'
    });
  };

  // 記録適用
  const handleApplyRecord = (record: any) => {
    // 日付をISO形式に変換（2024/12/27 → 2024-12-27）
    const convertDateToISO = (dateString: string) => {
      if (dateString.includes('/')) {
        const [year, month, day] = dateString.split('/');
    ...
    // 記録を再利用する場合は新規記録モードにする（selectedRecordをクリア）
    setSelectedRecord(undefined);
    
    toast.success(`${record.date}の記録を入力欄に反映しました`, {
      description: '新規記録として編集できます'
    });
  };
  ...
  const handleDiagnosisRegistration = (diagnosisData: any) => {
    console.log('病名登録データを受信:', diagnosisData);
