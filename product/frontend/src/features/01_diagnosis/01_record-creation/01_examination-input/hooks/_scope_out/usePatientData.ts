...
import type { Patient, ProgressRecord, HandoverItem, MedicalAlert, CurrentRecord } from "../types/patient-types";
import { 
  defaultPatient, 
  sampleProgressRecords, 
  sampleHandoverItems
} from "../src/constants/medical-data";
import { 
  isNewPatient, 
  getPatientAlerts, 
  getPatientById, 
...
export const usePatientData = () => {
  ...
  const addProgressRecord = (record: any) => {
    ...
    // シェーマIDを抽出
    const schemaIds = (record.soapRecord || '').match(/\[シェーマ:schema_(\d+)\]/g) || [];
    const schemas: { [key: string]: string } = {};
    
    // 各シェーマIDに対応する画像データをLocalStorageから取得
    schemaIds.forEach((match: string) => {
      const idMatch = match.match(/schema_(\d+)/);
      if (idMatch) {
        const schemaId = idMatch[1];
        const imageData = localStorage.getItem(`schema_${schemaId}`);
        if (imageData) {
          schemas[schemaId] = imageData;
          console.log(`シェーマ保存: schema_${schemaId}`);
        }
      }
    });
    ...
    const newRecord: ProgressRecord = {
      ...
      createdAt: now.toISOString(), // 記録作成日時を追加（ISO 8601形式）
      schemas: Object.keys(schemas).length > 0 ? schemas : undefined,  // シェーマ画像データを追加
    };
    setProgressRecords(prev => [newRecord, ...prev]);
  };

  // 経過記録の更新
  const updateProgressRecord = (recordId: string, record: any) => {
    // シェーマIDを抽出
    const schemaIds = (record.soapRecord || '').match(/\[シェーマ:schema_(\d+)\]/g) || [];
    const schemas: { [key: string]: string } = {};
    ...
    setProgressRecords(prev => 
      prev.map(item => {
        if (item.id === recordId) {
          // 既存の記録を更新（createdAtは保持、その他のフィールドを更新）
          return {
            ...item,
            ...record,
            id: recordId, // IDは変更しない
            createdAt: item.createdAt, // 作成日時は保持
