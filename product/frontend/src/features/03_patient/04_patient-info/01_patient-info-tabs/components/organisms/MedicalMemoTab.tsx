...
import { useMedicalMemo } from '../../hooks/useMedicalMemo';
import { usePatientInfoToast } from '../../hooks/useToast';
import { ja } from '@/shared/i18n/ja';

const t = ja.karte.patientInfo.medicalMemoTab;

interface MedicalMemoTabProps {
  records: MedicalMemoRecord[];
  /** staff は参照のみ */
  isReadOnly?: boolean;
}

export function MedicalMemoTab({ records, isReadOnly = false }: MedicalMemoTabProps) {
  const hook = useMedicalMemo(records);
      ...
      <section className="space-y-3">
        <SectionTable
          columns={[
            ...
            {
              key: 'isImportant',
              label: t.columns.isImportant,
              render: (row) => row.isImportant ? t.important : '',
            },
            { key: 'content', label: t.columns.content },
          ]}
          rows={hook.records}
