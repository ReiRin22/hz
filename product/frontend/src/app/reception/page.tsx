...
import { PatientReception } from './_components/templates/patient-reception';
import { UnifiedAppointments } from './_components/templates/unified-appointments';
import { QuestionnaireDialog } from './_components/organisms/questionnaire-dialog';
import { QuestionnaireInputDialog } from './_components/organisms/questionnaire-input-dialog';
import { ReceptionDialog } from './_components/organisms/reception-dialog';
import { WalkInReceptionDialog } from './_components/organisms/walk-in-reception-dialog';
import { PatientSearchDialog } from './_components/organisms/patient-search-dialog';

type ReceptionedPatient = {
  id: string;
  name: string;
  nameKana: string;
  patientNo: string;
  ...
  status: '受付済';
  hospitalStatus: 'waiting_for_examination' | 'waiting_for_consultation' | 'waiting_for_results' | 'waiting_for_accounting' | 'accounting_completed';
  order: number;
...
export default function App() {
  ...
  // 受付済み患者一覧（受付状況）
  const [receptionedPatients, setReceptionedPatients] = useState<ReceptionedPatient[]>([
    {
      id: '1',
      name: '佐藤花子',
      nameKana: 'サトウハナコ',
  ...
  const handleReceptionDialogReception = (data: any) => {
    ...
    const newReception = {
      ...
      consultationType: (data.consultationType || '初診') as '初診' | '再診',
      hasPostExamConsultation: false,
      receptionist: '受付', // TODO: 認証実装後ログインユーザーから取得
