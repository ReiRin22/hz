      ...
      const client = makeClient({
        acquireLock: vi.fn().mockRejectedValue(
          Object.assign(new Error('conflict'), {
            response: { status: 409, data: { error_code: 'LOCK_CONFLICT', locked_by_user_name: '別医師' } },
          }),
        ),
      });
      const service = makeService(client);
      ...
      const client = makeClient({
        acquireLock: vi.fn().mockRejectedValue(
          Object.assign(new Error('expired'), {
            response: { status: 409, data: { error_code: 'LOCK_EXPIRED' } },
          }),
        ),
      });
      const service = makeService(client);
      ...
      const client = makeClient({
        saveTestResults: vi.fn().mockRejectedValue(
          Object.assign(new Error('delete error'), {
            response: { status: 400, data: { error_code: 'VALIDATION_DELETE' } },
          }),
        ),
      });
      const service = makeService(client);
      ...
      const client = makeClient({
        saveTestResults: vi.fn().mockRejectedValue(
          Object.assign(new Error('format error'), {
            response: { status: 400, data: { error_code: 'UNKNOWN_CODE' } },
          }),
        ),
      });
      const service = makeService(client);
