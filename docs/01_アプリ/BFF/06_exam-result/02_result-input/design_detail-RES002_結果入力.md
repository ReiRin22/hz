...
| 9   | 採取日         | 日付       | 10        | YYYY/MM/DD         | 非活性      | 表示        |                      | -        | -                  | -                    | -                  | EVT_INIT01 レスポンスの testDate を自動設定（hasTestDate=true の場合のみ表示）。EVT_UI_01 で追加した行は null（hasTestDate=false・カレンダーで手入力可） |      |
...
| 2   | 検査コード検索欄 | 数値（実数） | 10        | 0～9999999999    | 活性        | 表示        |                                      | -        |              |                |                    | 追加したいコードで検索（項目名とAND検索も可能） |      |
...
| `availableUnits` | `UnitOption[]` | `[]` | EVT_INIT01 レスポンスの `availableUnits` を反映 | 画面離脱時 |
